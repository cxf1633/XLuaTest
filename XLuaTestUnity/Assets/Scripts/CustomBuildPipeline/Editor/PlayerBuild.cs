...
namespace CustomBuildPipeline.Editor
{
	public static class PlayerBuild
	{
		...
		{
			const string luaPackPath = "./LuaScripts.zip";
			if (File.Exists(luaPackPath))
			{
				File.Delete(luaPackPath);
			}

			ZipUtility.CompressFolderToZip(luaPackPath, Application.productName, $"{Application.dataPath}/../LuaScripts");
		}
		...
		public static void PackScriptAndBuildAddressableSteps(bool buildAddressable)
		{
			...
			PackLuaZip();
			File.Copy("./LuaScripts.zip", Application.streamingAssetsPath + "/LuaScripts", true);
			if (buildAddressable)
			{
				m_settings =
		...
		{
			PackLuaZip();
			File.Copy("./LuaScripts.zip", Application.streamingAssetsPath + "/BuiltInData", true);
		}
		...
		private static void BuildLuaScriptOnly()
		{
			const string luaPackPath = "./LuaScripts.zip";
			PackLuaZip();
			File.Copy(luaPackPath, "WinBuild/DBLike_Data/StreamingAssets/LuaScripts.zip", true);
		}
	}
}