        ...
        private void BeginOperation()
        {
            ...
            if (loadType == LoadType.Local)
            {
                if( Debug.isDebugBuild ) {
                    m_TransformedInternalId = m_TransformedInternalId.Replace('\\', '/');
                    var aaIndex = m_TransformedInternalId.IndexOf("/aa/", StringComparison.InvariantCulture);
                    if( aaIndex != -1 ) {
                        var abPath = m_TransformedInternalId.Substring(aaIndex);
                        var debugPath = $"{Application.persistentDataPath}{abPath}";
                        if( File.Exists(debugPath) ) {
                            m_TransformedInternalId = debugPath;
                        }                        
                    }
                    else {
                        Debug.LogWarning($"Path unknown {m_TransformedInternalId}.");
                    }
                }
