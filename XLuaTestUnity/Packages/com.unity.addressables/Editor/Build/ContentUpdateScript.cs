...
namespace UnityEditor.AddressableAssets.Build
{
    ...
    public struct AssetState : IEquatable<AssetState>
    {
        ...
        public bool Equals(AssetState other) {
            ...
            // return GitVersionStat.CheckAssetIsModified(guid) == false;
        }
    }
    ...
    public class CachedAssetState : IEquatable<CachedAssetState>
    {
        ...
        {
            bool result = other != null && GitVersionStat.CheckAssetIsModified(asset.guid) == 0;
            ...
            while (result && index < dependencies.Length) {
                ...
                var depAssetPath = AssetDatabase.GUIDToAssetPath(dep.guid);
                if( depAssetPath.StartsWith("Assets/Shader") ) {
                    result &= GitVersionStat.CheckAssetIsModified(dep.guid) != 2;
                }
                else {
                    result &= GitVersionStat.CheckAssetIsModified(dep.guid) == 0;
                }
                index++;
            }
    ...
    public static class ContentUpdateScript
    {
        ...
        public static Dictionary<AddressableAssetEntry, List<AddressableAssetEntry>> GatherModifiedEntriesWithDependencies(AddressableAssetSettings settings, string cachePath) {
            var dateTime = GetArg("-gitDate");
            if (string.IsNullOrEmpty(dateTime))
            {
                dateTime = "2023-1-16-16:00";
            }
            GitVersionStat.FetchGitLogAndCollect(dateTime);
