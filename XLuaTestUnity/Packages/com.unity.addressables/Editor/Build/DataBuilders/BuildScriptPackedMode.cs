...
namespace UnityEditor.AddressableAssets.Build.DataBuilders
{
    ...
    public class BuildScriptPackedMode : BuildScriptBase
    {
        ...
        private string GetShaderBundleName(string bundleName,bool hasExtern)
        {
            var name = bundleName;
            if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android)
            {
                name =hasExtern? "shaders_assets_all_6cb38d7220e2fd85c80e6f22e6e6626f.bundle":"shaders_assets_all_6cb38d7220e2fd85c80e6f22e6e6626f";
            }
               
            else if(EditorUserBuildSettings.activeBuildTarget == BuildTarget.iOS)
            {
                name =hasExtern? "9e284c7bdd6bb37fbe8f509a19a2ea06.bundle":"9e284c7bdd6bb37fbe8f509a19a2ea06";
            }
            return name;
        }
            ...
            AddressableAssetsBuildContext aaContext, Dictionary<string, string> bundleRenameMap, List<Action> postCatalogUpdateCallbacks)
        {
            ...
            for (int i = 0; i < builtBundleNames.Count; ++i)
            {
                ...
                if (outputBundleNames[0] == "shaders_assets_all.bundle")
                {
                    outputBundleNames[0] = GetShaderBundleName(outputBundleNames[0],true);
                }
            ...
            FileRegistry registry)
        {
                ...
                BundledAssetGroupSchema.BundleNamingStyle.NoHash)
            {
                postCatalogUpdates.Add(() =>
                {
                    ...
                    #if UNITY_ANDROID
                    var len = targetBundlePath.LastIndexOf('\\') + 1;
                    string bundlePathWithoutHash = targetBundlePath.Substring(0, len) + GetShaderBundleName("",true);
                    #elif UNITY_IOS
                    var len = targetBundlePath.LastIndexOf('/') + 1;
                    string bundlePathWithoutHash = targetBundlePath.Substring(0, len) + GetShaderBundleName("",true);
                    #else
                    ...
                    if (dataEntry != null)
                        if (DataEntryDiffersFromBundleFilename(dataEntry, bundlePathWithoutHash))
                            dataEntry.InternalId = StripHashFromBundleLocation(dataEntry.InternalId).Replace("shaders_assets_all",  GetShaderBundleName("",false));
                });
            }
        }
