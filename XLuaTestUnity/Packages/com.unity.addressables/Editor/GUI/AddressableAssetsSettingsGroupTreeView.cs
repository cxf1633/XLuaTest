...
namespace UnityEditor.AddressableAssets.GUI
{
    ...
    internal class AddressableAssetEntryTreeView : TreeView
    {
        ...
        void CellGUI(Rect cellRect, AssetEntryTreeViewItem item, int column, ref RowGUIArgs args)
        {
            ...
            switch ((ColumnId)column)
            {
                ...
                case ColumnId.BundleName:
                    if (item.entry != null && Event.current.type == EventType.Repaint)
                    {
                        var schema = item.entry.parentGroup.GetSchema<BundledAssetGroupSchema>();
                        if(schema == null)
                            break;
                        if (!schema.IncludeInBuild)
                        {
                            m_LabelStyle.Draw(cellRect, "Not In Clude In Build", false, false, args.selected, args.focused);
                            break;
                        }
                        var bundleMode = schema.BundleMode;
                        if (bundleMode == BundledAssetGroupSchema.BundlePackingMode.PackTogether)
                        {
                            var groupName = "Group_"+item.entry.parentGroup.name;
                            string name = AddressableExterns.GetBundleName(groupName);
                            if (string.IsNullOrEmpty(name))
                                name = item.entry.ReadOnly ? "" : "AA Map File is not exists,Please Rebuilt AA File";
                            m_LabelStyle.Draw(cellRect, name, false, false, args.selected, args.focused);
                        }
                        else
                        {
                            var path = item.entry.AssetPath;
                            string name = AddressableExterns.GetBundleName(path.ToLower());
                            if (string.IsNullOrEmpty(name))
                                name = item.entry.ReadOnly ? "" : "AA Map File is not exists,Please Rebuilt AA File";
                            m_LabelStyle.Draw(cellRect, name, false, false, args.selected, args.focused);
                        }
        ...
        static MultiColumnHeaderState.Column[] GetColumns()
        {
            ...
            retVal[counter].headerContent = new GUIContent("Bundule Name", "资源所在bundle 名字");
            retVal[counter].minWidth = 20;
            retVal[counter].width = 160;
            retVal[counter].maxWidth = 1000;
            retVal[counter].headerTextAlignment = TextAlignment.Left;
        ...
        void PopulateGeneralContextMenu(ref GenericMenu menu)
        {
            ...
            menu.AddItem(new GUIContent("Refresh AA Map"), false, RefreshAAMap);
        }
            ...
            else if (!hasReadOnly)
            {
                ...
                else if (isEntry)
                {
                    ...
                    if (selectedNodes.Count == 1)
                    {
                        ...
                        menu.AddItem(new GUIContent("Copy AssetBundleName to Clipboard"), false, CopyAssetBundleNameToClipboard, selectedNodes);
                    }
                    else if (selectedNodes.Count > 1)
                    {
                        menu.AddItem(new GUIContent($"Copy {selectedNodes.Count} AssetBundleName to Clipboard"), false, CopyAddressesToClipboard, selectedNodes);
                        menu.AddItem(new GUIContent("Copy " + selectedNodes.Count + " Addresses to Clipboard"), false, CopyAssetBundleNameToClipboard, selectedNodes);
                    }
        ...
        internal static void CopyAssetBundleNameToClipboard(object context)
        {
            List<AssetEntryTreeViewItem> selectedNodes = context as List<AssetEntryTreeViewItem>;
            string buffer = "";
            ...
            foreach (AssetEntryTreeViewItem item in selectedNodes)
            {
                ...
                if (schema.IncludeInBuild)
                {
                    ...
                    {
                        buffer += AddressableExterns.GetBundleName( "Group_"+item.entry.parentGroup.name) + ",";
                    }
                    else
                    {
                        buffer += AddressableExterns.GetBundleName( item.entry.address.ToLower()) + ",";
                    }
                }
