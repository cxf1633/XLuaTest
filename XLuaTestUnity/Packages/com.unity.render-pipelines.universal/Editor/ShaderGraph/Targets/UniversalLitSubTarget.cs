...
namespace UnityEditor.Rendering.Universal.ShaderGraph
{
    sealed class UniversalLitSubTarget : SubTarget<UniversalTarget>, ILegacyTarget
    {
        ...
        public override void GetFields(ref TargetFieldContext context)
        {
            ...
            var customAlpha = target.alphaMode == AlphaMode.Custom;
            var customZWrite = target.customZWrite;
            
            context.AddField(UniversalFields.SurfaceOpaque,       target.surfaceType == SurfaceType.Opaque && !customZWrite);
            context.AddField(UniversalFields.SurfaceTransparent,  target.surfaceType != SurfaceType.Opaque && !customZWrite);
            context.AddField(UniversalFields.BlendAdd,            target.surfaceType != SurfaceType.Opaque && target.alphaMode == AlphaMode.Additive && !customAlpha);
            context.AddField(Fields.BlendAlpha,                   target.surfaceType != SurfaceType.Opaque && target.alphaMode == AlphaMode.Alpha && !customAlpha);
            context.AddField(UniversalFields.BlendMultiply,       target.surfaceType != SurfaceType.Opaque && target.alphaMode == AlphaMode.Multiply && !customAlpha);
            context.AddField(UniversalFields.BlendPremultiply,    target.surfaceType != SurfaceType.Opaque && target.alphaMode == AlphaMode.Premultiply && !customAlpha);
            context.AddField(UniversalFields.BlendCustom,         customAlpha);
            ...
            context.AddField(UniversalFields.ZWriteCustom,      customZWrite);
            context.AddField(UniversalFields.CullCustom,      target.customCull);
            // Complex Lit
        ...
        public override void GetPropertiesGUI(ref TargetPropertyGUIContext context, Action onChange, Action<String> registerUndo)
        {
            ...
            context.AddProperty("Custom Z Write", new Toggle { value = target.customZWrite }, (evt) =>
            {
                if (Equals(target.customZWrite, evt.newValue))
                    return;

                registerUndo("Change Custom Z Write");
                target.customZWrite = evt.newValue;
                onChange();
            });
            
            
            context.AddProperty("Custom Cull", new Toggle { value = target.customCull }, (evt) =>
            {
                if (Equals(target.customCull, evt.newValue))
                    return;

                registerUndo("Change Custom Cull");
                target.customCull = evt.newValue;
                onChange();
            });
            ...
            context.AddProperty("Two Sided", new Toggle() { value = target.twoSided }, !target.customCull, (evt) =>
            {
